    ...
    try:
        ...
        if method == 'a':
            ...
            if data_type == 't':
                ...
                # Include first 25 characters of original filename in encrypted filename
                output_base = f"encrypted_{original_filename[:25]}"
...
def generate_lightning_qr(lnurl: str) -> str:
    """
    Generate Lightning Network QR code PNG file
    ...
    - Saves PNG file in program directory/.qrcode
    - User clicks link to open in image viewer
    
    Args:
        lnurl: Lightning payment address (must be 104 characters)
    
    Returns:
        Path to saved QR PNG file, or empty string on error
    """
    try:
        ...
        # Generate image with larger size for better viewing
        img = qr.make_image(fill_color='black', back_color='white')
        img = img.resize((500, 500))  # Resize to 500x500 for better display
        
        # Save to program directory/.qrcode
        qr_dir = os.path.join(BASE_DIR, '.qrcode')
        os.makedirs(qr_dir, exist_ok=True)
        qr_path = os.path.join(qr_dir, "lightning_qrcode.png")
        img.save(qr_path)
...
def open_qr_image(qr_path: str):
    """Display QR code image"""
    try:
        import subprocess
        if not os.path.exists(qr_path):
            print(color_error(f"QR code file not found: {qr_path}"))
            return
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                # Try eog
                try:
                    subprocess.Popen(['eog', qr_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                except:
                    print(color_info(f"QR saved at: {qr_path}"))
    except Exception as e:
        print(color_error(f"Error: {e}"))
...
• OpenSSL-compatible formats""")
        ...
        print(f"\n{color_bright('📱 Lightning Network QR Code:')}\n")
        ...
        if menu_choice == '1':
            if qr_path and os.path.exists(qr_path):
                ...
                safe_input(color_info("\n👉 Press Enter to return to Main Menu..."))
            else:
                print(f"\n{color_warning('⚠️  QR code not available')}\n")
        else:
            return
